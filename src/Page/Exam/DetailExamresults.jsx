...
function DetailExamresults() {
  ...
  const isAnswerCorrect = (question) => {
    return question.choiceUser?.every((choice) =>
      question.choiceCorrect?.includes(choice)
    );
  };

  return (
    <div className="w-[1400px] mx-5">
      <h2 className="text-3xl font-medium text-center my-5">
        Kết quả bài thi{" "}
      </h2>

      <Button
        className="font-medium mr-10"
        onClick={() => navigate("/adminpage/user-test")}
      >
        {" "}
      ...
      <div className="text-left my-5">
        <p className="text-base text-orange-500 font-medium">
          Nhận xét bài thi : {data?.data?.comment}
        </p>
        <p className="text-base text-orange-500 font-medium">
          Điểm số đạt được : {data?.data?.point}
        </p>
      </div>
      {data &&
          ...
          return (
            <div key={item.question.id} className="my-4">
              ...
              <div className="grid grid-cols-2">
                {item.question.listAnswer?.map((answer) => {
                  const userAnswerIsCorrect =
                    item.choiceUser?.includes(answer.answerKey) &&
                    item.choiceCorrect?.includes(answer.answerKey);
                  return (
                    <p
                      key={answer.id}
              ...
              {!correct && item.choiceUser?.length > 0 && (
                <div>
                  <p className="font-medium text-[#0014ff]">
                    Câu trả lời đúng:
                  </p>
                  {item?.question?.listAnswer
                    ...
                    ?.map((correctAnswer) => (
                      <p
                        ...
                        className="font-medium text-[#0014ff]"
                      >
